...
const disponibles = pieces.map((piece) => piece);
...
//Création de la liste d'abordables
...
for (let j = 0; j < disponibles.length; j++) {
  const pieceDispoElement = document.createElement("li");
  pieceDispoElement.innerText = `${disponibles[j].nom} - ${disponibles[j].prix}€`;
  disponiblesElements.appendChild(pieceDispoElement);
}
